...
fn get_repo_url() -> Option<String> {
    Some(
        match (
            ...
            (Ok(github_server), Ok(github_repo), _, _, _) => {
                format!("{github_server}/{github_repo}")
            }
            (_, _, Ok(ci_server_host), Ok(ci_project_namespace), Ok(ci_project_name)) => {
                format!("{ci_server_host}/{ci_project_namespace}/{ci_project_name}")
            }
            _ => return None,
        },
    )
