...
impl ModuleIf {
    #[must_use]
    pub(super) fn should_template(&self, allow_host_exec: bool) -> bool {
        #[cached(
            convert = r#"{ (mod_if.to_owned(), allow_host_exec) }"#,
            key = "(ModuleIf, bool)"
        )]
        fn inner(mod_if: &ModuleIf, allow_host_exec: bool) -> bool {
            match mod_if {
...
impl ModuleIfHostExec {
    fn should_template(&self) -> bool {
        {
            let c = cmd!(
                cd env::current_dir().unwrap_or_default();
                &self.cmd,
                for &self.args,
            );
